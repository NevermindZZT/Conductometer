...
void DYY_ShowDataScreen(void)
{
	...
	QPYLCD_DisplayCharacters(4, 69, BLACK, FONT16X16, 4, jrwdwht);
	...
	QPYLCD_DisplayCharacters(348, 82, BLACK, FONT16X16, 3, jrwdwht + 32 * 10);
	QPYLCD_DisplayString(396, 82, BLACK, FONT8X16, experimentalData.studentNumber);
	
	sprintf((char *)str, "%.1f", experimentalData.settedTemprature);
	QPYLCD_DisplayString(20, 95, BLACK, FONT8X16, str);	
	
	sprintf((char *)str, "%.1f", experimentalData.balanceTempeatrue);
	QPYLCD_DisplayString(20, 173, BLACK, FONT8X16, str);	
	
	sprintf((char *)str, "%.1f", experimentalData.heatingTempeatrue);
		...
		if (scanData != 0)
		{
			switch (scanData)
			{
				...
				case KEY_ENTER:																	//按键确认
					if ((KEYANDEC11_Scan()) == KEY_DEFALUT && (KEYANDEC11_Scan() == KEY_ENTER))	//长按
					{
						experimentalData.progress = COMPLETE;
						return;
					}
					break;
			}
...
{
	QPYLCD_DrawRectangle(132, 88, 216, 96, WHITE);
	
	QPYLCD_DrawLine(132, 88, 348, 88, QPYLCD_NewColor(4, 4, 2));
	...
	QPYLCD_DisplayCharacters(144, 100, BLACK, FONT24X24, 8, qdjrxyb);
	
	QPYLCD_DrawLine(144, 148, 193, 148, QPYLCD_NewColor(4, 4, 2));
	...
	QPYLCD_DrawLine(288, 148, 337, 148, QPYLCD_NewColor(4, 4, 2));
...
{
	QPYLCD_DrawRectangle(156, 88, 168, 96, WHITE);
	
	QPYLCD_DrawLine(156, 88, 324, 88, QPYLCD_NewColor(4, 4, 2));
	...
	QPYLCD_SetBackColor(WHITE);
	QPYLCD_DisplayCharacters(168, 100, BLACK, FONT24X24, 6, nywcsy);
	QPYLCD_DisplayCharacters(168, 148, BLACK, FONT24X24, 6, nywcsy + 72 * 6);
}
	...
	while (1)
	{
		...
		if (scanData != 0)
		{
			switch (scanData)
			{
				...
				case KEY_ENTER:
					experimentalData.progress = SHOWDATA;
					return;
//					break;
